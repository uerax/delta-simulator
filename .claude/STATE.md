...
- 优先级：P1
- 描述：
  1. **拓宽小地图视窗尺寸，消除多余空隙（已验证）**：
     - 根因排查：卡片可用内容总宽为 630rpx，原小地图视窗宽度仅 260rpx，而左侧信息栏（地图名称与地标胶囊）仅需约 200rpx 左右展示宽度，导致左右两侧被 `justify-content: space-between` 强制推开，中间产生 170rpx~200rpx 的空旷深色空白；
     - 落地方案：
       * 小地图视窗宽度从 `260rpx` 扩充至 `360rpx`（增幅约 38.5%），高度保持 `220rpx`，形成 1.64:1 的宽屏战术视野；
       * 左右行容器布局改为 `gap: 24rpx`，左侧信息列设 `min-width: 0` 与 `white-space: nowrap`，消除不必要的底部外边距，左右过渡紧凑精致，彻底消解中间巨大黑洞空档。
  2. **视窗几何居中与切片动态懒加载协同校准（已验证）**：
     - 同步更新 `miniprogram/pages/fortune/index.js` 中 `_getViewMetrics()` 的几何基准参数（`areaW: 360 * rpxRatio`）；
     - 确保 `_calculateMapOffset` 在 360rpx 新视窗下的初始地标绝对几何居中；
     - 确保 `_checkTilesInViewport` 在手势拖拽时以 360rpx 视窗精准判定可视切片图片加载，零黑边、零卡顿。
  3. **自动化测试 100% 绿色通过**：
     - 运势算法等测试全量通过。
  4. **版本号维护**：
     - 从 `v1.8.11` 递增至 `v1.8.12`。
- 涉及文件：
  - `miniprogram/pages/fortune/index.wxml`
  - `miniprogram/pages/fortune/index.wxss`
  - `miniprogram/pages/fortune/index.js`
...
## [2026-09-19] 重构运势启封卡片按钮布局与战术视觉体系 (解决拥挤与难看问题)
- 状态：已完成
- 优先级：P1
- 描述：
  1. **彻底解决按钮垂直贴合拥挤问题（已验证）**：
     - 根因排查：原启封卡片内两个 `<button>` 直接平铺在 flex 容器下，且均未定义 margin 与 gap，导致主次按钮 0 间距直接贴脸挤在一起，视觉局促厚重；
     - 落地方案：引入独立操作组容器 `.seal-btn-group`，声明 `width: 100%` 并建立 `gap: 22rpx` 的黄金呼吸间距；微调上方特性列表 `margin-bottom: 40rpx`，重构卡片上下视觉留白比例。
  2. **全面升级按钮战术视觉质感与主次层级（已验证）**：
     - **主按钮（开启今日鼠鼠运势）**：
       * 升级为三角洲机能炽火高燃渐变：`linear-gradient(135deg, #ff6b4a 0%, #e03a3e 55%, #c02227 100%)`；
       * 弃用原沉闷黑字，换用高对比纯白 `#ffffff` 搭配立体微暗投影 `text-shadow: 0 2rpx 4rpx rgba(0, 0, 0, 0.35)`；
       * 注入战术机能质感：顶部高光微描边 `inset 0 1.5rpx 1.5rpx rgba(255, 255, 255, 0.45)` 与弥散红色光晕 `box-shadow: 0 8rpx 24rpx rgba(224, 58, 62, 0.4)`；
       * 补全原缺失的 `.pulse-btn` 呼吸吸引微动效（`btnGlowPulse` 柔和缩放与光晕微扩），提升点击意愿；
       * 规范高度 `height: 88rpx`，重置 `margin: 0; padding: 0; line-height: 1;` 防止不同机型原生样式抖动变形。
     - **次按钮（返回大厅）**：
       * 告别同底色沉闷黑灰大实心砖块，重构为现代高质感半透明战术幽灵磨砂胶囊（Ghost Capsule）：`rgba(255, 255, 255, 0.05)` 与半透明微边框；
       * 高度收敛至 `74rpx`，字号 `26rpx`，搭配轻量返回引导符 `‹`，主次分明不抢戏；
       * 状态 A（未启封）与状态 B（已揭晓）统一应用新幽灵胶囊风格。
  3. **自动化测试 100% 绿色通过**：
     - 运势算法、用户管理等测试均通过，无语法报错。
  4. **版本号维护**：
     - 从 `v1.8.10` 递增至 `v1.8.11`。
- 涉及文件：
  - `miniprogram/pages/fortune/index.wxml`
  - `miniprogram/pages/fortune/index.wxss`
  - `package.json`
...
## [2026-09-19] 精简移除头像昵称编辑触发组件与已认证/UID标签模块
- 状态：已完成
- 优先级：P0
- 描述：
  1. **彻底移除点击头像和名称弹出使用微信头像/昵称的交互（已验证）**：
     - 根因对齐：按用户要求，彻底移除头像上的 `<button open-type="chooseAvatar">` 与铅笔标 `✎`，换为普通 `<view class="avatar-box">` 纯展示；
     - 彻底移除名称上的 `<input type="nickname">`，换为普通 `<text class="player-name">` 纯展示；
     - 弹窗内同步移除头像选择与昵称输入模块，不再出现任何“使用微信头像/使用微信昵称”提示，改为简洁的战术通行证登录提醒卡片与微信一键登录按钮。
  2. **彻底移除已认证与 UID 两个 Tag 模块（已验证）**：
     - 移除已登录态下的 `🛡️ 已认证` 标签；
     - 移除已登录态下的 `UID: usr_XXXXXX` 标签；
     - 移除未登录态下的 `UID: 待激活` 标签；
     - 界面子行仅展示纯净的 `今日已玩 X 局` 与未登录时的 `微信一键登录 ›`；
     - 清理对应已废弃的 WXSS 样式类。
  3. **技术限制与客观现状汇报准备**：
     - 微信官方自 2022 年底起全面下线静默获取微信真实头像昵称的 API；在去除 `open-type="chooseAvatar"` 与 `type="nickname"` 授权能力后，微信小程序物理层面无法静默抓取用户的真实微信头像与昵称，如实向用户说明该平台底层规则。
  4. **全套自动化测试 100% 绿色通过**：
     - 9 项单测套件全部通过。
  5. **版本号维护**：
     - 保持 `v1.8.10` 不变。
...
## [2026-09-19] 修复登录提醒主动弹出、微信头像/UID变化联动与历史无名勇士脏缓存自愈
- 状态：已完成
- 优先级：P0
- 描述：
  1. **打开小程序未登录主动弹出特勤登录提醒弹窗（已验证）**：
     - 根因排查：此前仅在顶部常驻微型游客卡片胶囊，无任何主动弹窗触发机制，首屏冷启动缺少引导；
     - 落地方案：在 `pages/index/index.js` `onShow` 生命周期增加会话级防骚扰检测（`_hasPromptedLogin`），未登录状态下冷启动延迟 350ms 自动升起战术暗黑磨砂风格的【特勤局通行证认证】弹窗，亦支持随时点击顶部游客栏呼出；
     - 弹窗内集成微信一键登录与通行证激活，并支持“以游客身份继续”，不阻塞玩家游戏流程。
  2. **登录后头像与特勤 UID 变化及直接引导（已验证）**：
     - 根因排查：微信官方自 2022 年底正式收回 `wx.getUserProfile` 静默读取微信头像与昵称能力，`wx.login` 仅返回凭证 `code`，导致此前登录后头像与昵称未产生视觉变化，且原界面未显式展示分配的特勤 UID；
     - 落地方案：
       * 顶部栏重构：已登录态显式展示绿色的 `🛡️ 已认证` 标签及蓝色特勤编号 `UID: usr_XXXXXX`；未登录态显示 `UID: 待激活` 与 `点击登录 ›`，对比鲜明；
       * 登录认证弹窗深度融合微信官方合规能力：内置 `<button open-type="chooseAvatar">`（点击直接调出微信官方头像选择器，一键更换微信头像）与 `<input type="nickname">`（点击键盘一键候选填充微信昵称）；
       * 登录流程中支持头像与昵称即时双向响应更新，登录成功后弹窗关闭，顶部头像、昵称、特勤 UID 全量高亮变化。
  3. **历史 Storage 缓存脏数据“无名勇士”强制自愈清洗（已验证）**：
     - 根因排查：老版本 Storage 中持久化存储了旧默认昵称 `'无名勇士'`，`getUserProfile` 解构赋值时将新代码的默认值覆盖；
     - 落地方案：在 `Storage.getUserProfile`、`Storage.setUserProfile` 与 `UserManager.init` 中增加强制自愈拦截机制，只要读取到 `'无名勇士'` 或空值，一律即刻重置为 `'野生鼠鼠'` 并自动回写落盘自愈。
  4. **全套自动化测试 100% 绿色通过**：
     - `scripts/verify_user_manager.js` 扩展测试用例 7，专门验证历史“无名勇士”缓存读取时的自愈与落盘；全部 7 项用例通过；
     - 9 项单测套件（用户管理、物资管理、运势算法、解耦架构、道具管理、地图背景、Planck 物理、爆汁特效、防吸附）100% 绿色通过。
  5. **版本号维护**：
     - 从 `v1.8.9` 递增至 `v1.8.10`。
- 涉及文件：
  - `miniprogram/utils/storage.js`
